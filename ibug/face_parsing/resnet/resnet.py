...
def conv3x3(
    ...
    dilation=1,
    mix_padding=False,
    padding_modes=("replicate", "circular"),
):
    """3x3 convolution with customised padding"""
    padding = dilation
    if mix_padding:
        padding = 0
    x = nn.Conv2d(
    ...
    if mix_padding:
        return nn.Sequential(MixPad2d([dilation, dilation], padding_modes), x)
    return x
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            MixPad2d((3,3)),
            nn.Conv2d(input_channel, self.inplanes, kernel_size=7, stride=2, padding=0,
                      bias=False)
        )
